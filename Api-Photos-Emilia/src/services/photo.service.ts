...
export const getPhotoUserId = (photoId: number) => {
	return prisma.photo.findMany({
		where: { id: photoId },
		select: {
			userId: true,
...
/**
 *
 * @param userId ID of the user to get their photos
 * @returns
 */

export const getPhotoUserIds = (userId: number) => {
	return prisma.photo.findMany({
		where: { id: userId },
		select: {
			userId: true,
 ...
 * @param userId ID of the user to delete their photo
 */
